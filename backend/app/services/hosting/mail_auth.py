...
class MailAuthService:
    ...
    def _dns_hints(self, domain: str, dkim_public: str | None) -> list[MailDnsHint]:
        ...
        hints = [
            MailDnsHint("MX", "@", MAIL_HOSTNAME, priority=10),
            MailDnsHint("TXT", "@", spf),
    ...
    async def _live_status(self, domain: str, dkim_public: str | None) -> dict:
        spf_ok, dkim_dns_ok, mx_ok = await asyncio.gather(
        ...
        if not spf_ok:
            messages.append(
                ...
                f"(replace Namecheap email-forwarding SPF)."
            )
        if not dkim_dns_ok:
            messages.append(
                f"Publish TXT {SELECTOR}._domainkey.{domain} with the DKIM public key."
            )
        if not mx_ok:
            messages.append(
                f"MX should point to {MAIL_HOSTNAME} so inbound mail reaches this server."
            )
        if ready:
    ...
    def _check_mx(domain: str) -> bool:
        try:
            answers = socket.getaddrinfo(domain, None)  # force resolution path exists
            _ = answers
        except OSError:
            pass
        try:
