...
class DomainService:
    ...
    async def provision_domain(self, domain_id: UUID) -> OperationResult:
        entity = await self._get_with_relations(domain_id)
        if entity is None:
            raise NotFoundError("Domain not found.")
    ...
    async def _provision_entity(self, entity: Domain) -> OperationResult:
        aliases: list[str] = []
        ...
        result = await self._provisioner.provision(
            ...
            create_docroot=entity.domain_type != "redirect",
