...
export async function streamAiChat(body: Record<string, unknown>): Promise<Response> {
...
export async function streamAiApply(token: string, confirmPassword?: string): Promise<Response> {
  return fetch(`${API_BASE_URL}/ai/actions/apply/stream`, {
    method: 'POST',
    headers: authHeaders(),
    body: JSON.stringify({
      token,
